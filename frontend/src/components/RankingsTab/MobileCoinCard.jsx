 ...
 */
export function MobileCoinCard({ coin, onSelect, onInspect, onInspect3D, index }) {
  ...
  return (
    ...
    >
      ...
      {/* Badges strip: 3D Studio, CTO, Boosts, Dex Paid, Smart Money, KOL */}
      <div className="flex items-center gap-1.5 flex-wrap mb-2">
        <button
          ...
          onClick={(e) => {
            ...
            if (onInspect3D) onInspect3D(coin);
            else if (onInspect) onInspect(coin);
            else if (onSelect) onSelect(coin);
          }}
          className="inline-flex items-center gap-1 font-bold px-1.5 py-0.2 rounded text-[10px] font-mono border text-cyan-300 bg-cyan-950/60 border-cyan-500/40 active:scale-95 cursor-pointer shadow-sm"
          title="Open in 3D 360° Studio Turntable"
        >
          <span>🎮</span>
          <span>3D</span>
        </button>
        ...
        <span className={`inline-flex items-center gap-1 font-bold px-1.5 py-0.2 rounded text-[10px] font-mono border ${
          (coin.kolCount || 0) > 0
            ? 'text-amber-300 bg-amber-950/60 border-amber-500/40 shadow-sm'
            : 'text-slate-400 bg-[#121824] border-[#222c3d]'
        }`}>
          <span>⭐</span>
          <span>{(coin.kolCount || 0) > 0 ? `${coin.kolCount} KOL${(coin.kolCount || 0) === 1 ? '' : 's'}` : '0 KOLs'}</span>
        </span>
      </div>
